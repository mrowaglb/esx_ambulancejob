	...
	if not isUsing then
		if _type == 'bsmall' then
			if health < 200 then 
				...
				exports['CloudRP']:DrawProcent(150, function()
					Citizen.InvokeNative(0x6B76DC1F3AE6E6A3, playerPed, newHealth)
		...
		elseif _type == 'bmedium' then
			...
			exports['CloudRP']:DrawProcent(300, function()
				FreezeEntityPosition(playerPed, false)
				ClearPedTasks(playerPed)
				isUsing = false
				Citizen.InvokeNative(0x6B76DC1F3AE6E6A3, playerPed, maxHealth)
...
function RemoveItemsAfterRPDeath()
	CreateThread(function()
		...
		ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()			
			...
			DoScreenFadeIn(800)
			TriggerServerEvent('cloud_logs:triggerLog', "Przeteleportował się na szpital: "..hospital, 'teleportszpital')
		end)
	end)
end
...
function DeathFunc() 
	...
	CreateThread(function ()
		...
		if weapon == `WEAPON_UNARMED` or ((weapon == `WEAPON_RUN_OVER_BY_CAR` or damagedbycar) and sourceofdeath ~= playerPed) or weapon == `WEAPON_NIGHTSTICK` then
			obezwladniony = true
			CreateThread(function() 
				exports["cloud_taskbar"]:taskBar(knockoutDuration, "Odzyskujesz siły...", false, true)
			end)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_interaction',
			{
				...
				if not IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
					...
					elseif data.current.value == 'hijack' then
						if(not IsPedInAnyVehicle(playerPed)) then
							TriggerServerEvent('cloud:pay', 1500)
							menu.close()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'ambulance_wear' then
			...
			if PlayerData.job.name == 'offambulance' then
				TriggerServerEvent('cloud:setJob', 'ambulance', true)
		...
		if data.current.value == 'citizen_wear' then
			...
			if PlayerData.job.name == 'ambulance' then
				...
				TriggerServerEvent('cloud:setJob', 'ambulance', false)
			end
...
AddEventHandler('hypex_ambulancejob:hypexrevive', function(notBlock)
	...
	if IsDead and not notBlock then
		TriggerServerEvent('cloud_wypadanie:bron')
...
function OpenInventoryMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'items', {
		...
		if data.current.value == 'get' then
			TriggerEvent('cloud:getInventoryItem', 'society_ambulance')
		elseif data.current.value == 'put' then
			TriggerEvent('cloud:putInventoryItem', 'society_ambulance')
		end
	end, function(data, menu)
		menu.close()
		CurrentAction		= 'items_menu'
...
function OpenInventoryMenu2()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'items', {
		...
		menu.close()
		if data.current.value == 'get' then
			TriggerEvent('cloud:getInventoryItem', 'society_sams2')
		elseif data.current.value == 'put' then
			TriggerEvent('cloud:putInventoryItem', 'society_sams2')
		end
	end, function(data, menu)
		menu.close()
		CurrentAction		= 'items2_menu'
